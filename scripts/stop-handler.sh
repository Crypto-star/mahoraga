...
COMPLETED=$(echo "$STATE" | jq -r '.completed // false' 2>/dev/null)
...
if [ "$VALIDATION_STATUS" -eq 0 ]; then
    ...
    MANDATORY_PASSED=$(echo "$VALIDATION_RESULT" | jq -r '.mandatory.passed // 0' 2>/dev/null)
    MANDATORY_REQUIRED=$(echo "$VALIDATION_RESULT" | jq -r '.mandatory.required // 1' 2>/dev/null)
    BONUS_SCORE=$(echo "$VALIDATION_RESULT" | jq -r '.bonus.score // 0' 2>/dev/null)
    BONUS_AVAILABLE=$(echo "$VALIDATION_RESULT" | jq -r '.bonus.available // 4' 2>/dev/null)
...
DETAILS=$(echo "$VALIDATION_RESULT" | jq -r '.details // ""' 2>/dev/null)
